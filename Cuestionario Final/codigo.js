...
function Opcion_Seleccionada() {
  ...
  for (let i = 0; i < radioButtons.length; i++) {
      if (radioButtons[i].checked) {
          respuestas[posicion] = i; // Se guarda la respuesta en la posición correcta
      }
  }
}
...
function Marcar_Opcion_Seleccionada() {
  const radioButtons = document.getElementsByName('opciones');
  desmarcarTodos(); // Se desmarcan todas antes de marcar la seleccionada
...
function avanzar() {
  Opcion_Seleccionada(); // Guarda la respuesta antes de avanzar
...
function atras() {
  Opcion_Seleccionada(); // Guarda la respuesta antes de retroceder
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Pequeño retraso para asegurar que la animación se vea correctamente
  setTimeout(function() {
      // Inicia la animación del agua bajando
      waveContainer.classList.add('hidden');
      
      // Mostrar el contenido después de que el agua baje
      setTimeout(function() {
          content.classList.add('visible');
      }, 400); // Mostrar contenido a mitad de la animación de agua
  }, 100);
});
